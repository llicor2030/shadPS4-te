...
TileManager::~TileManager() {
    for (const auto& [capacity, buffer] : free_scratch_buffers) {
        vmaDestroyBuffer(instance.GetAllocator(), buffer.first, buffer.second);
    }
}

vk::DeviceSize TileManager::ScratchCapacity(const vk::DeviceSize size) {
    // Round up to a power of two so images of a similar scale share one allocation.
    constexpr vk::DeviceSize MinCapacity = 64 * 1024;
    return std::bit_ceil(std::max(size, MinCapacity));
}

void TileManager::ReleaseScratchBuffer(const ScratchBuffer buffer, const vk::DeviceSize size) {
    if (free_scratch_buffers.size() >= MAX_POOLED_SCRATCH_BUFFERS) {
        vmaDestroyBuffer(instance.GetAllocator(), buffer.first, buffer.second);
        return;
    }
    free_scratch_buffers.emplace_back(ScratchCapacity(size), buffer);
}

TileManager::ScratchBuffer TileManager::GetScratchBuffer(vk::DeviceSize size) {
    const vk::DeviceSize capacity = ScratchCapacity(size);
    for (auto it = free_scratch_buffers.begin(); it != free_scratch_buffers.end(); ++it) {
        if (it->first == capacity) {
            const auto buffer = it->second;
            free_scratch_buffers.erase(it);
            return buffer;
        }
    }
    size = capacity;
...
TileManager::Result TileManager::DetileImage(Buffer& in_buffer, u32 in_offset,
    ...
    scheduler.DeferOperation([this, out_buffer, out_allocation, output_size]() {
        ReleaseScratchBuffer({out_buffer, out_allocation}, output_size);
    });
...
void TileManager::TileImage(Image& in_image, std::span<vk::BufferImageCopy> buffer_copies,
    ...
    // Only the tiling path needs the host-side offsets, so build them after the early return.
    ...
    scheduler.DeferOperation([this, temp_buffer, temp_allocation, host_buffer_size]() {
        ReleaseScratchBuffer({temp_buffer, temp_allocation}, host_buffer_size);
    });
