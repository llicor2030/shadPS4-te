...
VsyncLock::VsyncLock(std::chrono::nanoseconds guest_period,
    ...
    drop_missed = EmulatorSettings.IsVsyncDropMissedVblanks();

#ifdef _WIN32
    enabled = present_mode_allows_lock && EmulatorSettings.IsVsyncLockEnabled();
#else
    enabled = false;
#endif
    if (!enabled) {
    ...
    margin_ns = std::clamp(EmulatorSettings.GetVsyncLockMarginMs(), 1u, 50u) * NsPerMs;
    log_stats = EmulatorSettings.IsVsyncLockLogEnabled();
